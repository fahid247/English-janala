...
function spinner(trigger) {
    if (trigger == true) {
        ...
        document.querySelector(".cardsContainer").classList.add("hidden")
    }
    else {
...
function findWords(data) {
    ...
    const matchedWords = data.filter(element => element.word.toLowerCase().includes(value));

    showLevelWord(matchedWords); 
}
